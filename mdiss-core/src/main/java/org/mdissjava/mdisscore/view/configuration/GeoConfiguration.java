...
public class GeoConfiguration implements Serializable {
		...
		if (country != null && !country.equals("")) {
			...
			for (int i = 0; i < Lestados.size(); i++) {
				System.out.println("Added to the List: "
						+ Lestados.get(i).getNombre());
				states.put(Lestados.get(i).getNombre(),
						Short.toString(Lestados.get(i).getId()));
			}
		} else {
			states = new TreeMap<String, String>();
			cities = new TreeMap<String, String>();
	...
	public void AjaxCityChange() {
		...
		System.out.println("Ciudad: " + c.getNombre()
				+ " seleccionada con coordenadas: " + this.getCoordx() + " , "
				+ this.getCoordy());
	}

	public void displayLocation() {
		FacesMessage msg = new FacesMessage("Selected", "Country:" + country
	...
	public String doSave() throws ServletException, IOException {
		// System.out.println("Valores de cidauda, stado y pais: "+this.getCity()+" , "+
		// this.getState() +" , "+this.getCountry());
		if (!this.getCountry().equals("0") && !this.getState().equals("0")
				&& !this.getCity().equals("0")) {
			...
			FacesContext.getCurrentInstance().addMessage(null,
					new FacesMessage("Geographical settings updated"));
		} else {
